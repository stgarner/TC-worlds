...
function get_grid_version()
  ...
  if(grid_version<0) then
    if(field_exist(grid_file, 'geolon_t', no_domain=.true.)) then   !stg
       grid_version = VERSION_0 
    else if(field_exist(grid_file, 'x_T', no_domain=.true.)) then
       grid_version = VERSION_1
    else if(field_exist(grid_file, 'ocn_mosaic_file', no_domain=.true.) ) then
...
subroutine get_grid_size_for_all_tiles(component,nx,ny)
  ...
  select case (get_grid_version())
  case(VERSION_0,VERSION_1)
     call field_size(grid_file, varname1, siz, no_domain=.true.)
...
subroutine get_grid_comp_area(component,tile,area,domain)
  ...
  case(VERSION_2) ! mosaic gridspec
     ...
     area(:,:) = 0.
     if(field_exist(grid_file,xgrid_name, no_domain=.true.)) then
        ! get the number of the exchange-grid files
        call field_size(grid_file,xgrid_name,siz, no_domain=.true.)
        n_xgrid_files = siz(2)
        found_xgrid_files = 0
